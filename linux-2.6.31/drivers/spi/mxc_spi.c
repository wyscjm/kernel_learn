...
extern void spi_chipselect_active(int cspi_mod, int active_level);
extern void spi_chipselect_inactive(int cspi_mod, int active_level);
...
void mxc_spi_chipselect(struct spi_device *spi, int is_active)
{
	...
	if (is_active == BITBANG_CS_INACTIVE) {
		/*Need to deselect the slave */
                //取消片选
		spi_chipselect_inactive(spi->chip_select & MXC_CSPICTRL_CSMASK, ((spi->mode & SPI_CS_HIGH) != 0));
		return;
	}

	//片选有效
        spi_chipselect_active(spi->chip_select & MXC_CSPICTRL_CSMASK, ((spi->mode & SPI_CS_HIGH) != 0));
