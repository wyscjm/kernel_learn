...
#if 0		//观察以太网接收数据
			int t = 0;
			printk("get data len: %d \n", pkt_len - 4);
			for (t = 0; t < pkt_len - 4; t++) {
				printk("0x%02x ", *(data + t));
				if ((t%12) == 0)
					printk("\n");
			}
			printk("\n");
...
static phy_info_t const phy_info_ksz8041NL = {		/* 板上的phy芯片是ksz8041NL */
	.id = 0x0022151,
	.name = "KSZ8041NL",
	.config = phy_cmd_ks8721bl_config,
	.startup = phy_cmd_ks8721bl_startup,
