...
static inline void rs485_direction(uart_mxc_port *up, int w)
{       
        struct serial_rs485_zlg *conf = up->port.private_data;
        if (!conf) 
                return;
        if (w) { /* send */
                if (conf->transmit) { 
                        conf->transmit(); 
                        if (conf->delay_before_send)
                                udelay(conf->delay_before_send);
                }
        } else {
                if (conf->receive) {
              //  while( !(serial_in(up, UART_LSR) & UART_LSR_TEMT) );
                        if (conf->delay_after_send)
                                udelay(conf->delay_after_send);
                        conf->receive();
                }
        }
...
static unsigned int mxcuart_tx_empty(struct uart_port *port)
{
	...
	rs485_direction((uart_mxc_port *) port, 0);
