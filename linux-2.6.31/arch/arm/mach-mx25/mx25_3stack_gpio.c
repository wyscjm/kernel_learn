...
*/
	...
	case 3:
		...
		mxc_iomux_set_input(MUX_IN_UART4_IPP_UART_RXD_MUX,
				    INPUT_CTL_PATH0);
#else
		mxc_request_iomux(MX25_PIN_KPP_COL0, MUX_CONFIG_ALT1); 	/* RXD				*/
		mxc_request_iomux(MX25_PIN_KPP_COL1, MUX_CONFIG_ALT1);	/* TXD				*/
...
#else
	mxc_request_iomux(MX25_PIN_UART2_RTS, MUX_CONFIG_ALT5); /* FEC_RESET */
	mxc_iomux_set_pad(MX25_PIN_UART2_RTS, 0);

	gpio_request(IOMUX_TO_GPIO(MX25_PIN_UART2_RTS), "uart2_RTS");
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_UART2_RTS), 0); /* FEC_RESET_B */

	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_UART2_RTS), 0);	/* FEC_RESET_B */
	mdelay(20);
	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_UART2_RTS), 1);	/* FEC_RESET_B */
#endif
...
//片选信号
void spi_chipselect_active (int cspi_mod, int active_level)
{
	active_level = (active_level != 0)? 1 : 0;
	switch (cspi_mod) {
		case 0x00://SS0 操作
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS0), active_level);
		break;

		case 0x01://SS1 操作
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS1), active_level);
		break;
...
EXPORT_SYMBOL(spi_chipselect_active);

//取消片选信号
void spi_chipselect_inactive (int cspi_mod, int active_level)
{
	int inactive_level = (active_level != 0)? 0 : 1;
	switch (cspi_mod) {
		case 0x00://SS0 操作
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS0), inactive_level);
		break;

		case 0x01://SS1 操作
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS1), inactive_level);
		break;
...
EXPORT_SYMBOL(spi_chipselect_inactive);
...
#define SPI_PAD_CTL2 (PAD_CTL_HYS_SCHMITZ|PAD_CTL_PKE_ENABLE| \
	...
	case 0:
		...
		mxc_request_iomux(MX25_PIN_CSPI1_SS1, MUX_CONFIG_ALT5);
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS0), "CSPI1_ss0");
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS0), 1);
                gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS1), "CSPI1_ss1");
                gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS1), 1);
		...
		#endif
	//	mxc_request_iomux(MX25_PIN_VSTBY_ACK, MUX_CONFIG_ALT2); /*SS3*/		/* 被模拟i2c使用		*/
...
#endif
	//	mxc_iomux_set_pad(MX25_PIN_VSTBY_ACK, SPI_PAD_CTL1);			/*　被模拟i2c使用		*/
	...
	case 0xFF:
		...
		mxc_request_iomux(MX25_PIN_CSPI1_SS0,  MUX_CONFIG_GPIO);
		mxc_request_iomux(MX25_PIN_CSPI1_SS1,  MUX_CONFIG_GPIO);
		mxc_request_iomux(MX25_PIN_CSPI1_SCLK, MUX_CONFIG_GPIO);
		mxc_request_iomux(MX25_PIN_CSPI1_RDY,  MUX_CONFIG_GPIO);

		mxc_iomux_set_pad(MX25_PIN_CSPI1_MOSI, 0);
		mxc_iomux_set_pad(MX25_PIN_CSPI1_MISO, 0);
		mxc_iomux_set_pad(MX25_PIN_CSPI1_SS0,  0);
		mxc_iomux_set_pad(MX25_PIN_CSPI1_SS1,  0);
		mxc_iomux_set_pad(MX25_PIN_CSPI1_SCLK, 0);
		mxc_iomux_set_pad(MX25_PIN_CSPI1_RDY,  0);

		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_MOSI), "spi_mosi");
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_MISO), "spi_miso");
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS0),  "spi_sso");
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SS1),  "spi_ss1");
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_SCLK), "spi_sclk");
		gpio_request(IOMUX_TO_GPIO(MX25_PIN_CSPI1_RDY),  "spi_rdy");
#endif
	default:
		break;
	}
...
#define CAN_PAD_IN_CTL (PAD_CTL_HYS_CMOS | PAD_CTL_PKE_NONE)
	...
	case 0xFF:
		...
		mxc_request_iomux(MX25_PIN_GPIO_C, MUX_CONFIG_FUNC); /*TXCAN*/
		mxc_request_iomux(MX25_PIN_GPIO_D, MUX_CONFIG_FUNC); /*RXCAN*/

		mxc_iomux_set_pad(MX25_PIN_GPIO_C, 0);
		mxc_iomux_set_pad(MX25_PIN_GPIO_D, 0);

		//gpio_request(IOMUX_TO_GPIO(MX25_PIN_GPIO_C), "GPIO_C");
		//gpio_request(IOMUX_TO_GPIO(MX25_PIN_GPIO_D), "GPIO_D");
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_GPIO_C), 0);
		gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_GPIO_D), 0);
		break;
...
void M257_system_gpio_active(void)
{
	/*
	 * 初始化LED引脚
	 */
	mxc_request_iomux(MX25_PIN_ECB, MUX_CONFIG_ALT5);		/* ERR LED GPIO3_23		*/
	mxc_request_iomux(MX25_PIN_LBA, MUX_CONFIG_ALT5);		/* RUN LED GPIO3_24		*/

	mxc_iomux_set_pad(MX25_PIN_ECB, PAD_CTL_PKE_ENABLE | PAD_CTL_HYS_SCHMITZ | PAD_CTL_DRV_1_8V);
	mxc_iomux_set_pad(MX25_PIN_LBA, 0);

	gpio_request(IOMUX_TO_GPIO(MX25_PIN_ECB), "ERR_LED");
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_LBA), "RUN_LED");

	//gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_ECB), 1); 
	gpio_direction_input(IOMUX_TO_GPIO(MX25_PIN_ECB));
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_LBA), 1); 

	/*
	 * 蜂鸣器引脚
	 */
	mxc_request_iomux(MX25_PIN_PWM, MUX_CONFIG_ALT5);
	mxc_iomux_set_pad(MX25_PIN_PWM, 0);
	gpio_request(IOMUX_TO_GPIO(MX25_PIN_PWM), "beep");
	gpio_direction_output(IOMUX_TO_GPIO(MX25_PIN_PWM), 0); 

}
EXPORT_SYMBOL(M257_system_gpio_active);

void M257_err_led_ctl(int value)
{
//	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_ECB), !value);
	if (value) {
...
EXPORT_SYMBOL(M257_err_led_ctl);

void M257_run_led_ctl(int value)
{
	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_LBA), value);
}
EXPORT_SYMBOL(M257_run_led_ctl);

void M257_beep_ctl(int value)
{
	gpio_set_value(IOMUX_TO_GPIO(MX25_PIN_PWM), value);
}
EXPORT_SYMBOL(M257_beep_ctl);
